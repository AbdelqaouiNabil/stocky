          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                ...
                <!-- Purchase -->
                <b-col lg="4" md="4" sm="12" class="mb-3">
                  <validation-provider name="Purchase" :rules="{ required: true}">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Purchase') + ' ' + '*'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        @input="Selected_Purchase_Ref"
                        :disabled="details.length > 0"
                        v-model="purchase_return.purchase_id"
        ...
        if (this.details[i].detail_id == id) {
          if (detail.quantity - 1 > 0) {
            if (detail.quantity - 1 > detail.purchase_quantity) {
              this.makeToast(
                "warning",
                this.$t("qty_return_is_greater_than_qty_purchased"),
                this.$t("Warning")
              );
            } else {
              this.formatNumber(this.details[i].quantity--, 2);
